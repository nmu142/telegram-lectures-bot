...
async def db_connect() -> aiosqlite.Connection:
    ...
    async def _integrity_ok(c: aiosqlite.Connection) -> bool:
        try:
            ...
            return True  # if not supported, don't block
    ...
    try:
        ...
        if not _is_db_malformed_error(e):
            try:
                await conn.close()
            except Exception:
                pass
    ...
    # Recovery from malformed DB (best-effort) using backup + remove WAL/SHM
    try:
        await conn.close()
    except Exception:
        pass
...
if __name__ == "__main__":
    try:
        ...
        raise
>>>>>>> ab9a563ec212c4885845d51b84c9b09f2f399bdc
